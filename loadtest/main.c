...
#include "load.h"
//stm32 time start -> init, and throught irq_handle, setbit and clear flag
//x86 throught function run,how to again enter timer 
...
static enum hrtimer_restart monitor_handler(struct hrtimer *hrtimer)
{
    ...
    check_load();
    hrtimer_forward_now(hrtimer,ms_to_ktime(100));
    return ret;
}
...
static void start_timer(void)
{
    ...
    hrtimer_start_range_ns(&timer,ms_to_ktime(100),0,HRTIMER_MODE_REL_PINNED);
}
